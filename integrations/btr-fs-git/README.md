...
## bdfs integration — DwarFS compression for push/pull

btr-fs-git optionally integrates with [btrfs-dwarfs-framework](https://gitlab.com/openos-project/linux-kernel_filesystem_deving/btrfs-dwarfs-framework) to compress snapshots as DwarFS images during push/pull transfers. This can significantly reduce transfer size for large read-only snapshots.

### How it works

Instead of piping a raw `btrfs send` stream over SSH, the bdfs path:
1. Compresses the snapshot locally as a `.dwarfs` image (`bdfs snapshot demote`)
2. Transfers the `.dwarfs` file via `scp`
3. Decompresses it on the remote side (`bdfs snapshot promote`)

Falls back to plain `btrfs send/receive` automatically when `bdfs` is not installed on either end.

### Usage

Import the patch module before calling `fire.Fire(BtrFsGit)` to enable the `--compress-via-bdfs` flag:

```python
import btrfsgit.btrfsgit_bdfs_patch  # noqa: F401  (side-effects only)
```

Then use the flag on push or pull:

```bash
# Push with DwarFS compression
bfg push /subvol /snapshot /remote/subvol --COMPRESS_VIA_BDFS=true

# Pull with DwarFS compression
bfg pull /remote/snapshot /local/subvol --COMPRESS_VIA_BDFS=true
```
...
| File | Purpose |
|---|---|
| `btrfsgit/bdfs_transfer.py` | `export_to_dwarfs()`, `import_from_dwarfs()`, `bdfs_push()`, `bdfs_pull()` |
| `btrfsgit/btrfsgit_bdfs_patch.py` | Monkey-patch adding `COMPRESS_VIA_BDFS` to `BtrFsGit.push` and `BtrFsGit.pull`; idempotent |

### Requirements

- `bdfs` binary installed on both local and remote machines
- `scp` available for `.dwarfs` file transfer

When `bdfs` is absent on either end, the flag is silently ignored and plain `btrfs send/receive` is used.
