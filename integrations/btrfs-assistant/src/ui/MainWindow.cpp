...
#include "ui/BdfsPartitionsTab.h"
    ...
    : QMainWindow(parent), m_ui(new Ui::MainWindow), m_btrfs(btrfs), m_btrfsMaint(btrfsMaintenance), m_snapper(snapper)
{
    ...
    // BDFS tab — gracefully absent when the bdfs daemon is not running
    m_bdfsClient = new BdfsClient(this);
    m_bdfsPartitionsTab = new BdfsPartitionsTab(m_bdfsClient, this);
    m_ui->tabWidget_mainWindow->addTab(m_bdfsPartitionsTab, tr("BDFS"));
